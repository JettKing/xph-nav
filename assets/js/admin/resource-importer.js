...
const normalizeSourceName=v=>{
  let x=txt(v)
    .replace(/^github\s*[-:|｜]\s*[^/\s]+\/([^\s]+)$/i,'$1')
    .replace(/^github\s*[-:|｜]\s*/i,'')
    .replace(/^gitlab\s*[-:|｜]\s*/i,'')
    .replace(/\s*(?:—|–|-|·|•|\||｜)\s*(?:the\s+)?(?:official\s+)?(?:website|github)\s*$/i,'')
    .trim();
  const parts=x.split(/\s*(?:—|–|\||｜|·|•)\s*|:\s+/).map(v=>v.trim()).filter(Boolean);
  if(parts.length>1&&parts[0].length>=2&&parts[0].length<=48)x=parts[0];
  x=x.replace(/\s+(?:微信公众?号|AI知识库|工作流|Agent平台|RAG大模型.*)$/i,'').trim();
  return x;
};
...
function assertResourceSchema(r){const allowed=CONTRACT.resourceSchema.required;if(Object.keys(r).some(k=>!allowed.includes(k)))throw new Error('资源包含 Contract 禁止字段');for(const key of allowed){if(!(key in r))throw new Error(`JSON字段缺失：${key}`)}if(!validClassification(r.category,r.subcategory))throw new Error('分类/子分类ID不符合统一词库');const mapped=iconForSubcategory(r.subcategory);if(!mapped||r.icon!==mapped)throw new Error('Icon未按统一子分类映射');if(!r.description||count(r.description)!==16)throw new Error('最终简介未通过16字验收');if(!Array.isArray(r.capabilities)||r.capabilities.some(v=>!TAGS.capabilities.includes(v)))throw new Error('能力词不在标准词库');if(!Array.isArray(r.scenarios)||r.scenarios.some(v=>!TAGS.scenarios.includes(v)))throw new Error('场景词不在标准词库');if(!r.attributes||!Array.isArray(r.attributes.platform)||!Array.isArray(r.attributes.language)||!Array.isArray(r.attributes.audience)||!TAGS.attributes.pricing.includes(r.attributes.pricing))throw new Error('attributes 不符合 Contract');return r}
...
async function readStep(){
  // 只有用户在本次页面会话中主动编辑过名称，才视为人工覆盖；浏览器自动填充/旧草稿值不得污染真实来源识别。
  const manualName=state.manualNameDirty?normalizeSourceName($('#resourceName').value):'';
...
async function aiStep(){
  ...
  try{
    ...
    setLoading(STAGES[2],'正在生成并严格校验核心简介…');
...
function saveDraft(){const r=assertResourceSchema(buildResource()),all=JSON.parse(localStorage.getItem(KEY)||'[]'),i=all.findIndex(x=>x.id===r.id);if(i>=0)all[i]=r;else all.unshift(r);localStorage.setItem(KEY,JSON.stringify(all));loadDrafts();status('审核草稿已保存。','ok')}
...
function nativeJs(r){return `,
${JSON.stringify(assertResourceSchema(r),null,4)}
`}
...
function bind(){
  const vocabularyHealth=window.XPHVocabularyValidator?.validateTaxonomy();if(vocabularyHealth&&!vocabularyHealth.valid){status('词库 Contract 校验失败：'+vocabularyHealth.errors.join('；'));return;}
