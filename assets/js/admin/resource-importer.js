...
(function(){
  ...
  function autoDescription16(input){
    ...
    if(/chatgpt|claude|gemini|对话|chat|assistant/.test(hay))return exact16("智能对话内容生成创作辅助工具平台");
    if(/midjourney|stable diffusion|flux|image generation|图片生成|绘图|绘画/.test(hay))return exact16("图像生成视觉创作设计辅助工具平台");
    if(/video|视频生成|runway|kling|sora/.test(hay))return exact16("视频生成内容创作剪辑辅助工具平台");
    if(/audio|music|音乐|voice|语音/.test(hay))return exact16("音频语音生成创作辅助工具服务平台");
    if(/search|搜索|perplexity|research|检索/.test(hay))return exact16("搜索信息研究分析辅助工具服务平台");
    if(/code|coding|编程|developer|开发|github|copilot|cursor/.test(hay))return exact16("编程代码开发调试辅助工具服务平台");
    if(/automation|自动化|workflow|工作流/.test(hay))return exact16("自动化流程效率管理辅助工具平台型");
    if(/note|notion|笔记|knowledge|知识管理/.test(hay))return exact16("知识笔记团队协作管理工具服务平台");
    if(/marketing|营销|广告/.test(hay))return exact16("营销策划内容创作推广辅助工具平台");
    if(/design|设计|figma|ui|ux/.test(hay))return exact16("设计创作界面协作视觉辅助工具平台");
    if(/course|课程|learning|学习/.test(hay))return exact16("学习课程知识获取训练辅助资源平台");
  ...
  async function fetchPage(url){
    ...
    return {
      title,
      resourceName:name,
      description:exact16(chineseDescription({name,url:clean,description,content:body,title,keywords})),
  ...
  async function init(){
    ...
    $("#analyzeBtn").onclick=async()=>{
      ...
      if(!current.description || autoDescription){
        input.description=exact16(meta.description||autoDescription16({...input,...meta}));
    ...
    $("#fetchBtn").onclick=async()=>{
      ...
      try{
        ...
        if(!current.description || autoDescription){
          const value=exact16(m.description||autoDescription16({name:$("#resourceName").value,url,...m}));
