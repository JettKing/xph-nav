/** 徐胖虎资源社 Renderer v5.2：渲染资源、统计、分页与状态。 */
window.ResourceRenderer = {
    ...
    render({container="#resource-list",data=[]}={}){
        ...
        this.updateCount(ResourceStore.getFilteredCount(),ResourceStore.getState().page,ResourceStore.getPageCount());
        this.renderPagination(); this.refresh();
    },
    updateCount(count,page=1,pages=1){const el=document.getElementById("resourceCount");if(el)el.textContent=pages>1?`${count} 个资源 · ${page}/${pages}页`:`${count} 个资源`;},
    renderPagination(){
