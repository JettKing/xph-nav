/**
 * XPH Resource Contract V5.3
 * Single source of truth for the deployed resource shape and import protocol.
 * No legacy/compatibility fields are permitted.
 */
export const XPH_RESOURCE_CONTRACT = Object.freeze({
  version: '5.3-core-contract-2',
  ...
  legacyFields: Object.freeze(['platform','pricing','language','audience','subcategories','desc','project','tags','_meta','features']),
  resourceSchema: Object.freeze({
    '$schema':'https://json-schema.org/draft/2020-12/schema',
    type:'object', additionalProperties:false,
    required:['id','name','description','icon','thumbnail','category','subcategory','website','github','capabilities','scenarios','attributes','official','recommend','status'],
    properties:{
      id:{type:'string',minLength:1}, name:{type:'string',minLength:1}, description:{type:'string',minLength:16,maxLength:16}, icon:{type:'string',minLength:1}, thumbnail:{type:'string'},
      category:{type:'string',pattern:'^[a-z][a-z0-9_]*$'}, subcategory:{type:'string',pattern:'^[a-z][a-z0-9_]*$'}, website:{type:'string'}, github:{type:'string'},
      capabilities:{type:'array',items:{type:'string'}}, scenarios:{type:'array',items:{type:'string'}},
      attributes:{type:'object',additionalProperties:false,required:['pricing','platform','language','audience'],properties:{pricing:{type:'string'},platform:{type:'array',items:{type:'string'}},language:{type:'array',items:{type:'string'}},audience:{type:'array',items:{type:'string'}}}},
      official:{type:'boolean'}, recommend:{type:'boolean'}, status:{type:'string'}
    }
  })
});
