...
6. description 如果没有人工简介，必须恰好16个字符；中文为主体，可以自然使用少量英文字母或数字，例如“AI”“K线”，不能整句全英文；必须来自真实网页/GitHub内容；优先提炼“核心对象 + 核心功能/用途 + 结果”，不要写空话。输出前必须按字符逐个编号1、2、3……16自检；中文、英文字母、数字、标点均各计1字符，不能靠换行或空格规避长度。
...
12. 生成后在内部逐字符检查description；AI生成时必须正好16字符并包含中文。不要把标点、空格或换行视为可忽略字符。分类必须逐项检查是否存在于合法词库。
...
若没有人工简介：先在内部拟定多个候选简介；对每个候选按实际字符逐个计数，只有计数恰好16的候选才允许进入最终JSON。推荐采用“对象+核心功能+用途/结果”的完整句式；不要因为短而直接输出8-12字的标题式短语。最终只输出恰好16字符的核心简介。严禁为了凑16字在末尾添加“工具、平台、软件、资源、应用、专业、强大、实用”等空泛词。
若有人工简介，原样保留。
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      const retryStrategies = [
        '',
        '\n这是第2次独立生成。上一轮未通过长度验收。请完全重新理解原始资料，先在内部写出至少3个不同的16字符候选，再逐字符计数，只选择真正自然、完整、最贴合资料的一条输出。禁止复用上一轮答案。',
        '\n这是第3次独立生成。请重新从真实网页/GitHub内容提炼“对象+核心功能+用途/结果”。内部逐字符编号1-16后再输出。英文字母、数字、中文均各计1字符。禁止机械补“工具/平台/软件/资源/应用/专业/强大/实用”。',
        '\n这是第4次独立生成。优先使用真实资料中的具体功能词，不要追求营销表达。先内部生成候选，再删改到恰好16字符且语义完整。绝对不要复制或续写之前的答案。',
        '\n这是第5次独立生成。最后一次验收模式：重新理解全部原始资料，内部生成多个候选并逐字符计数；最终 description 必须恰好16字符、中文为主体、自然完整、有真实信息密度。不要输出解释，不要沿用任何上一轮答案。'
      ];
      const retryHint = retryStrategies[attempt - 1] || '';
