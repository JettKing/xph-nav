...
export async function onRequestPost({request,env}){
  ...
  for(let attempt=1;attempt<=MAX_ATTEMPTS;attempt++){
    try{
      const strategy=[
        '优先准确表达资源最核心的实际功能，生成5条互不重复候选。',
        '重新独立理解原始资料，不参考任何上一轮答案；优先使用具体功能词。',
        '重新独立生成5条候选；先写自然完整语义，再逐字符调整到16字，禁止机械补词。',
        '从真实资料中挑最有信息量的功能与用途，生成5条不同表达；不要复制SEO。',
        '最后一次独立生成：只保留真实、自然、完整、16字的候选；宁可换一种表达，也不要沿用上一轮。'
      ][attempt-1];
      const result=await callDeepSeek(env,[{role:'system',content:exactSystem},{role:'user',content:`${exactBase}\n\n这是第${attempt}次独立生成。${strategy}\n输出5条候选JSON。`}]);
      const candidates=Array.isArray(result.parsed?.candidates)?result.parsed.candidates.map(clean):[clean(result.parsed?.description)];
      const valid=candidates.find(isValid16);
      if(valid)return new Response(JSON.stringify({description:valid,category,subcategory,model:result.data?.model||MODEL,attempt,candidateCount:candidates.length}),{status:200,headers});
      lastError=new Error(`第${attempt}次生成的5个候选均未通过16字符质量校验`);
    }catch(e){lastError=e;}
  }
  return new Response(JSON.stringify({error:`DeepSeek连续${MAX_ATTEMPTS}次未生成合格的16字符简介，请重试。${lastError?.message?`（${lastError.message}）`:''}`}),{status:422,headers});
}